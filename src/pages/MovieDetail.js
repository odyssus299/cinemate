...
export const MovieDetail = ({apiPath}) => {
  ...
  useEffect(() => {
    async function fetchMovie() {
      const response = await fetch(`https://api.themoviedb.org/3/movie/${parameters.id}?api_key=37bb68c9cc33fa1179f4bf27881a6afb`);
        ...
        <div className="max-w-2xl text-gray-700 text-lg dark:text-white">
          ...
          <p className="my-7 flex flex-wrap gap-2">
            {movie.genres && movie.genres.length > 0 && movie.genres.map((genre) => {
               return <span className="mr-2 border rounded border-2 border-gray-200 dark:border-gray-600 p-2" key={genre.id}>{genre.name}</span>
            })}
          </p>
          <div className="flex items-center">
              ...
              <span className="text-gray-900dark:text-white">{movie.vote_count} reviews</span>
          </div>
